...
from .const import DOMAIN, EXCLUDED_ARG_KEYS, LOG_PREFIX, PLATFORMS
...
def build_ukbcd_args(config_data: dict) -> list:
    ...
    if not council:
        _LOGGER.error(
            f"{LOG_PREFIX} No council or original_parser specified in configuration"
        )
        raise ConfigEntryNotReady(
            "Missing council specification in configuration. Please reconfigure this integration."
        )
